...
///
/// The returned PTY master fds must be kept alive by the caller for as long as the
/// container is expected to run; dropping them closes the terminal.
pub fn run_container_with_console(
    ...
    let pty_fds = handle_console_socket(stream);
