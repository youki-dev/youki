...
impl Container {
    ...
    pub fn restore(&mut self, opts: &RestoreOptions) -> Result<(), LibcontainerError> {
        match self.do_restore(opts) {
            Ok(()) => Ok(()),
            Err(err) => {
                // OCI spec Errors section requires that on failure the operation MUST leave
                // the system in the same state as if it had never been attempted. Doing this
                // outside do_restore() covers every early return, and guarantees
                // do_restore()'s cleanup guard has already run so the state directory no
                // longer holds a live bind mount.
                let cleanup_err = self.cleanup_failed_restore().err();
                Err(RestoreContainerError::new(err, cleanup_err).into())
            }
        }
    }

    /// Remove the container state directory created by `build_for_restore()`.
    fn cleanup_failed_restore(&self) -> Result<(), LibcontainerError> {
        // do_restore() unmounts criu-root and removes the (then empty) directory, so
        // anything left here means the bind mount of the rootfs may still be live.
        // remove_dir_all() does not stop at mount points and would follow it, deleting
        // the container's rootfs.
        let criu_root = self.root.join(CRIU_ROOT_DIR);
        if criu_root.exists() {
            tracing::error!(
                ?criu_root,
                root = ?self.root,
                "skipping container state directory cleanup because criu-root still exists"
            );
            return Err(LibcontainerError::Other(format!(
                "{criu_root:?} still exists, refusing to remove the container state directory"
            )));
        }

        remove_dir_all(&self.root).map_err(|err| {
            tracing::error!(
                ?err,
                root = ?self.root,
    ...
    fn do_restore(&mut self, opts: &RestoreOptions) -> Result<(), LibcontainerError> {
