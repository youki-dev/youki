...
impl FromStr for MountOption {
    ...
    fn from_str(option: &str) -> std::result::Result<Self, Self::Err> {
        match option {
            ...
            "private" => Ok(MountOption::Private(false, MsFlags::MS_PRIVATE)),
            "rprivate" => Ok(MountOption::Rprivate(
                false,
                MsFlags::MS_PRIVATE | MsFlags::MS_REC,
            )),
            "shared" => Ok(MountOption::Shared(false, MsFlags::MS_SHARED)),
            "rshared" => Ok(MountOption::Rshared(
                false,
                MsFlags::MS_SHARED | MsFlags::MS_REC,
            )),
            "slave" => Ok(MountOption::Slave(false, MsFlags::MS_SLAVE)),
