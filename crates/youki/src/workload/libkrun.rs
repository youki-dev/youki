...
impl Krun {
    fn load() -> Result<Self, ExecutorError> {
        unsafe {
            let lib = Library::new(LIBKRUN_NAME)
                .map_err(|e| ExecutorError::Other(format!("load {LIBKRUN_NAME}: {e}")))?;
...
impl LibkrunExecutor {
    fn get_or_load_krun(&self) -> Result<Rc<Krun>, ExecutorError> {
        if let Some(krun) = self.lib.get() {
            return Ok(krun.clone());
        }
        let krun = Rc::new(Krun::load()?);
...
impl Executor for LibkrunExecutor {
    fn pre_exec(&self, spec: Spec) -> Result<Spec, ExecutorError> {
        ...
        let krun = self.get_or_load_krun()?;
    ...
    fn exec(&self, spec: &Spec) -> Result<(), ExecutorError> {
        ...
        let log_level = read_krun_log_level(spec).unwrap_or(DEFAULT_LOG_LEVEL);
        tracing::debug!(log_level = log_level, "using krun log level");
        krun.set_log_level(log_level)?;

        let (vcpus, ram_mib) = read_krun_vm_config(spec);
...
fn read_krun_vm_config(spec: &Spec) -> (u8, u32) {
...
fn read_krun_log_level(spec: &Spec) -> Option<u32> {
    let ann = spec.annotations().as_ref()?;
    let s = ann.get("krun.log_level")?;
    let log_level = s.trim().parse::<u32>().ok()?;
    tracing::debug!(
        source = "annotations",
        log_level = log_level,
        "log level selected"
