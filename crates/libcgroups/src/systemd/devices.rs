...
pub enum SystemdDevicesError {
    ...
    #[error(
        "systemd DeviceAllow is a whitelist and cannot express the deny rule {0}; use the \
         cgroupfs cgroup manager for this configuration"
    )]
    UnsupportedDenyRule(String),
...
impl Devices {
    fn apply_devices(
        ...
        // A spec with no device rules must not end up unconfined here while the cgroupfs
        // path (v2::devices::controller) and runc confine it.
        ...
        // cannot express that, and this manager only sets unit properties, so dropping the
        // rule would hand out access the spec refuses. runc skips deny rules and lets the
        // eBPF filter it attaches carry the exact rules, replacing the one systemd derives
        // from DeviceAllow (ebpf_linux.go), yet runc 1.3.4 still fails such a container
        // while it starts.
        //
        // TODO: support them the way crun does, by handing systemd an own program through
        // its BPFProgram= property instead of going through DeviceAllow.
        if let Some(rule) = emulator.rules.iter().find(|rule| !rule.allow()) {
            return Err(SystemdDevicesError::UnsupportedDenyRule(rule.to_string()));
        }

        // Everything allowed and nothing denied is exactly DevicePolicy=auto.
        if emulator.default_allow {
            properties.insert("DevicePolicy", Variant::String("auto".to_string()));
            // An empty array is the only way to clear a unit's existing DeviceAllow list.
        ...
        for rule in &emulator.rules {
            ...
            let type_prefix = match typ {
                ...
                // 'u' and 'p' have no device cgroup equivalent; type='a' never reaches here.
                // runc refuses them while reading the spec and the eBPF path errors on them
                // (v2::devices::program), so dropping them quietly would be the odd one out.
                _ => {
                    return Err(SystemdDevicesError::UnsupportedDeviceType(typ));
                }
            };

            // runc's wildcard is -1 and the eBPF path reads it the same way, so a spec keeps
            // its meaning whichever cgroup manager applies it.
            let major = rule.major().filter(|major| *major >= 0);
            let minor = rule.minor().filter(|minor| *minor >= 0);

            // Emulator::add_rule() drops rules without access; "rwm" is for any other caller.
            let access = rule.access().as_deref().unwrap_or("rwm");

            let path = match (major, minor) {
                (None, None) => format!("{type_prefix}-*"),
                (None, Some(minor)) => {
                    // systemd names either a whole device group or one node, so widening this
                    // would allow every device of the type. runc skips it too.
                    tracing::warn!(
                ...
                (Some(major), Some(minor)) => format!("/dev/{type_prefix}/{major}:{minor}"),
            };

            allowed.entry(path).or_default().add(access);
        }

        // An entry with no access is read as "rwm" by systemd, so leave those out.
        ...
        // policy without one.
        //
        // NOTE: systemd appends to an existing DeviceAllow list and only an empty array
        // clears it, and properties are passed as a map, so entries would accumulate if
        // apply() ran twice on the same unit. runc sends an empty array first to avoid that.
        ...
        if in_section {
            ...
            if let Some((maj_str, name)) = trimmed.split_once(' ') {
                if let Ok(maj) = maj_str.trim().parse::<u64>() {
                    let name = name.trim();
                    // A major can be listed twice, as "4 /dev/vc/0" and "4 tty" are. Only a
                    // group name works as a DeviceAllow entry, and it needs the device type
                    // as a prefix. Ref: runc findDeviceGroup(), systemd.exec(5) DeviceAllow=.
...
";
    ...
    fn try_apply(
        ...
        Devices::apply_devices(&opt, systemd_version, &mut props)?;
        Ok(props)
    }
    ...
    #[test]
    fn test_blacklist_with_deny_rule_is_rejected() {
        // Dropping the deny rule here would leave every device allowed.
        ...
        let deny_char = LinuxDeviceCgroupBuilder::default()
            ...
            .build()
            .unwrap();
        let err = try_apply(&resources(vec![allow_all, deny_char]), 245).unwrap_err();

        assert!(
            matches!(err, SystemdDevicesError::UnsupportedDenyRule(_)),
            "expected UnsupportedDenyRule, got {err:?}"
        );
    }
    ...
    fn test_exact_device_node() {
        ...
        assert!(
            allow.contains(&entry("/dev/block/8:0", "rw")),
            "expected /dev/block/8:0 rw in allow list, got {allow:?}"
        );
    }
    ...
    fn test_negative_major_is_a_wildcard() {
        // runc's wildcard is -1 and the eBPF path reads it the same way, so a spec means
        // the same thing whichever cgroup manager applies it.
        let rule = LinuxDeviceCgroupBuilder::default()
            .typ(LinuxDeviceType::C)
            .major(-1_i64)
            .allow(true)
    ...
    fn test_unsupported_device_type_is_rejected() {
        // 'u' and 'p' have no device cgroup equivalent. runc refuses them when reading the
        // spec, and the eBPF path errors on them, so they must not be dropped silently.
    ...
    fn test_major_only_old_systemd_uses_device_group() {
        // systemd < 240 only understands the names in /proc/devices, where major 1 is the
        // memory device group on every Linux system.
        let rule = LinuxDeviceCgroupBuilder::default()
            .typ(LinuxDeviceType::C)
            .major(1_i64)
            .allow(true)
            ...
            .build()
            .unwrap();
        let props = apply(&resources(vec![rule]), 239);
    ...
    fn test_deny_rule_is_rejected() {
        ...
        let deny_write = LinuxDeviceCgroupBuilder::default()
            ...
            .build()
            .unwrap();
        let err = try_apply(&resources(vec![allow_null, deny_write]), 245).unwrap_err();
    ...
    #[test]
    fn test_find_device_group_reports_a_missing_file() {
        let err = find_device_group("/proc/does-not-exist", LinuxDeviceType::C, 1).unwrap_err();
